...
namespace sict {
			...
			if (iteminfo[i].itemName == item.getName()) {
				if (item.getQuantity() < 1) {
					...
					os << " Unable to fill " << this->Name << "[" << this->getNameProduct() << "]["
						<< this->iteminfo->itemName << "][" << this->iteminfo->SerialNumber << "]" << " out of stock" <<std::endl;
				}
				else if (iteminfo[i].fill) {
					iteminfo[i].SerialNumber = item.getSerialNumber();
					os << " Unable to fill " << this->Name << "[" << this->getNameProduct() << "]["
						<< this->iteminfo->itemName << "][" << this->iteminfo->SerialNumber << "]" << " already filled" <<std::endl;
				}
				else {
					...
					os << " Filled " << this->Name << "[" << this->getNameProduct() << "]["
						<< this->iteminfo->itemName << "][" << this->iteminfo->SerialNumber << "]" << std::endl;
	...
	void CustomerOrder::display(std::ostream& os, bool showDetail) const {
		...
		if (showDetail) {		
			for (size_t i = 0; i < itemNumber; i++) {
				std::string status = (iteminfo[i].fill) ? "Filled" : "Not Filled";
